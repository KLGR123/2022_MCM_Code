function [B, P, C, S, tree, tree_num, tree_type, v1, v2, v3, value, klist, Qlist, q, P_mean, B_mean] = Forest_decision(type, SI, k, Q, size, w, year, grow_type, cut_type, shape, tree_pro, rain, humid, wind, forcast_depth, weight)
    ...
    % rain humid wind 平均降雨 平均湿度 平均风速
    % ------------------------
    % 输出变量
    ...
    v3 = zeros(1, year);
    value = zeros(1, year);
    P_mean = zeros(1, year + 3);
    B_mean = zeros(1, year + 3);
    S_mean = zeros(1, year + 1);
    ...
    % ------------------------
    % 初始化森林生物量指标 Biomass
    tree_num = zeros(3, year*3); % 统计每年三种树龄对应树的数目
    B = zeros(1, year*2); % 森林总生物量
    ...
    for ii = 1 : year
        % ------------------------
        % 计算 value 和更新 k Q mean
        klist(ii) = k;
        Qlist(ii) = Q(1);
        P_mean(ii) = mean(P(1:ii));
        B_mean(ii) = mean(B(1:ii));
        S_mean(ii) = mean(S(1:ii));

        if ii <= 2 % 计算差分值
            B_diff = B(ii);
            P_diff = P(ii);
        else
            B_diff = B(ii-1) - B(ii-2);
            P_diff = P(ii-1) - P(ii-2);
        end

        [v1i, v2i, v3i, valuei] = value_score(B_diff, tree, tree_num, rain, humid, wind, P_diff, tree_type, w, weight);
        v1(ii) = v1i; 
        v2(ii) = v2i;
        v3(ii) = v3i;
        value(ii) = valuei;
        ...
        if mod(mod_ii, k) == 0 % 判断到达采伐年
            ...
            jj = 1;
            while jj <= resize
                if mark == floor(count1 * Q(1)) % 到达采伐强度要求 + count2 * Q(2)
                    break; % 停止采伐
                end
            ...
            % ------------------------
            % 再调用决策器，判断是否改变现有 Management Plan
            [k_new, Q_new] = decision(tree, tree_num, rain, humid, wind, tree_type, w, weight, forcast_depth, size, c, shape, cut_type, p, P, T, TP, B, ii);
            if k_new ~= k
                k = k_new;
                Q = Q_new;
                mod_ii = 0; % 重新开始整除的计数
                disp('transition point detected!');
            end
        end
